...
namespace Activation {
    ...
    inline std::vector<double> softmax(const std::vector<double>& x) {
        ...
        double max_elem = *std::max_element(x.begin(), x.end()); // for numerical stability
        double sum = 0.0;
        for (double val : x) {
    ...
    inline std::vector<double> softmax_derivative(const std::vector<double>& softmax_output) {
        // simplified version: returns only diagonal elements of the Jacobian
        // for full derivative, it is better to use the Jacobian matrix (s_i * (δ_ij - s_j))
...
public:
    ...
    // vector-matrix multiplication (vector length must match number of matrix rows)
